...
test('should set type of variable by contextual type', () => {
  const sourceFile = project.createSourceFile('write-access.ts', 'import path from "path";\nconst a = fun();\npath.join(a);', {overwrite: true});
...
test('should set type of variable by contextual type with union type', () => {
  const sourceFile = project.createSourceFile('write-access.ts', 'function b(b: number | string | boolean);\nconst a = "asd";\nb(a);', {overwrite: true});
  sourceFile.getDescendantsOfKind(SyntaxKind.VariableDeclaration)
  TypesRefactor.inferContextualType(sourceFile);
