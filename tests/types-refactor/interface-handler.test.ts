...
test('should create interface and set type of variable with object assignment', () => {
  ...
  expect(flatten(interfaceDeclaration)).toEqual('export interface A { auth?: { a: string; b: number; } | undefined; }');
  expect(sourceFile.getText()).toEqual(`let a: ${TypeHandler.getType(interfaceDeclaration!).getText()} = true ? { auth: { a: "a", b: 1 } } : {};`);
});
...
test('should create interface and replace object union type with interface', () => {
  ...
  const generatedFile = getSourceFile(project, "");
  const interfaceDeclaration = generatedFile.getInterface(i => i.getName() === "A");
  expect(flatten(interfaceDeclaration)).toEqual('export interface A { a?: string | number | undefined; b?: string; c?: number; }');
  expect(sourceFile.getText()).toEqual(`let a: ${TypeHandler.getType(interfaceDeclaration!).getText()};`);
});
...
test('should create interface and replace object union type with interface 2', () => {
  ...
  expect(sourceFile.getText()).toEqual(`let a: import("path").PlatformPath | ${TypeHandler.getType(interfaceDeclaration!).getText()} | undefined;`);
});
