...
test('should refactor standard require', () => {
  const sourceFile = project.createSourceFile(
    'standard-require.ts',
    'const content = require("ts-morph");',
    { overwrite: true },
  );
...
test('should refactor standard require with object destructuring assignment', () => {
  const sourceFile = project.createSourceFile(
    'standard-require.ts',
    'const {content1, content2} = require("ts-morph");',
    { overwrite: true },
  );
  ImportsRefactor.requiresToImports(sourceFile);
...
test('should refactor simple name binding require', () => {
  const sourceFile = project.createSourceFile(
    'standard-require.ts',
    'const {name: rename} = require("ts-morph");',
    { overwrite: true },
  );
  ImportsRefactor.requiresToImports(sourceFile);
...
test('should refactor invalid computed property binding require', () => {
  const sourceFile = project.createSourceFile(
    'standard-require.ts',
    'const value = "name";\nconst {[value]: rename} = require("ts-morph");',
    { overwrite: true },
  );
  ImportsRefactor.requiresToImports(sourceFile);
