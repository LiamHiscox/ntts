...
class ImportClauseRefactor {
  ...
  private static refactorDefaultImport = (
    importStatement: ImportDeclaration,
  ...
  private static refactorNamedImports = (
    importStatement: ImportDeclaration,
    usedImportNames: string[],
    importedSourceFile: SourceFile,
    sourceFile: SourceFile,
  ...
  private static getExportedList = (importedSourceFile: SourceFile): string[] => importedSourceFile
    .getExportDeclarations()
    .reduce((exportedNames: string[], exportDeclaration) => {
  ...
  private static createObjectDestructuring = (
    importStatement: ImportDeclaration,
    defaultImports: ImportSpecifier[],
    initializer: string,
    sourceFile: SourceFile,
  ) => {
    const index = importStatement.getChildIndex();
    const objectDestructuring = defaultImports.map((i) => (i.getAliasNode() ? `${i.getName()}: ${i.getAliasNode()}` : i.getName())).join(', ');
  ...
  private static hasDefaultExport = (sourceFile: SourceFile): boolean => !!sourceFile.getDefaultExportSymbol();
}

export default ImportClauseRefactor;
