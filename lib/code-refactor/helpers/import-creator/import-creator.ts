...
class ImportCreator {
  ...
  static addImport = (nameNode: ObjectBindingPattern | Identifier, moduleSpecifier: string, sourceFile: SourceFile) => {
    const importDeclaration = ImportFinder.getNonNamespaceImportDeclaration(moduleSpecifier, sourceFile);
    if (Node.isIdentifier(nameNode)) {
      this.addDefaultImportStatement(importDeclaration, nameNode, moduleSpecifier, sourceFile);
    } else {
  ...
  private static getNamedImports = (objectBinding: ObjectBindingPattern): string[] => objectBinding
    .getElements()
    .map((binding) => {
