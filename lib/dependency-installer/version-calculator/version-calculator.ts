export class VersionCalculator {
    ...
    private static closestTargetVersion = (versionList: number[][], targetVersion: number, index: number): number => {
        return versionList.reduce((closest: number, version) => {
    ...
    private static limitTargetVersion = (versionList: number[][], index: number, condition: (...values: number[]) => number): number => {
        return condition(...versionList.map(version => version[index]));
    }

    private static versionList = (version: string): number[] => {
        return version.split('.').map(v => +v);
    }

    private static validVersion = (version: string): boolean => {
        return /^\d+\.\d+\.\d+$/.test(version);
    }

    private static filterByClosestVersion = (versions: number[][], target: number, index: number): number[][] => {
        return versions.filter(version => version[index] === target);
    }
